...
def statistics(request):
    ...
    comp = Company
    vouchers =Voucher.objects.all().order_by('voucher_name')
    ...
    if 't_id' in request.session:
        ...
        if request.method=='POST':
            ...
            if debit == credit:
                ...
                voucher = Voucher.objects.get(company = comp,voucher_name = jname)
                voucher.no_of_vouchers += 1
                voucher.save()
...
def statistics(request):
    ...
    if 't_id' in request.session:
        ...
        sg_count=CreateStockGrp.objects.filter(company = comp).count()
